...
export function clearChannelPersona(channelId: string) {
  localStorage.removeItem(personaStorageKey(channelId));
}

export function clearLastRouteIfMatches(route: string) {
  if (getLastChannelRoute() === route) {
